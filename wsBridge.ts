...
function connect(port: number) {
  ...
  try {
    ...
    ws.on('open', () => {
      ...
      // Keepalive ping. Clear any interval from a previous connection first —
      // otherwise each reconnect leaks another recurring timer, since the
      // old interval's `ws?.readyState === 1` check reads this module-level
      // `ws` variable, which now points at the *new* connection and is
      // almost always open, so the old interval's clearInterval() branch
      // is never reached.
