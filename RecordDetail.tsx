...
export default function RecordDetail({ record, onClose }: RecordDetailProps) {
  ...
  // Reset to the loading state only when switching to a *different* record
  // (not on every WS update of the same record).
  useEffect(() => {
    setFull(null)
    setLoading(true)
  }, [record.id])

  // Fetch full details whenever the record is first selected, and re-fetch
  // in the background whenever the live record summary advances
  // (record.updated_at changes on every pipeline step) so the panel
  // doesn't go stale while the user has it open.
  useEffect(() => {
