...
from core.models.inference_engine import (
    detect_inference_profile, apply_quantization_to_model_id, QuantizationTier,
)
...
# Tracks which agent role is making the current complete() call, so that
# complexity-aware routing (e.g. speculative decoding) can be applied.
#
# This MUST be a ContextVar rather than an attribute on the shared
# ModelRouter singleton. Multiple pipeline records are processed
# concurrently as separate asyncio Tasks that all share one ModelRouter
# instance. With a plain instance attribute, BaseAgent.run() sets
# `router._current_agent = self.name` and then `await`s an event emission
# before calling complete() — during that await, a different task could set
# the attribute to its own agent name, so complete() would read the wrong
# agent for this call. ContextVar values are isolated per-Task (each Task
# gets its own copy of the context), so this race cannot happen.
...
class ModelRouter:
    ...
    async def complete(
        ...
        # ── 6. Speculative decoding (complexity-aware, Ollama only) ─────────
        # Only HIGH complexity agents (reasoning, config_agent) use draft model
        # Simple agents skip draft overhead — not worth it for lightweight tasks
        _use_spec = self._speculative_enabled and warm_pool.should_use_speculative(
            CURRENT_AGENT.get()
        )
        if model.startswith("ollama/"):
            options = _build_ollama_options(model, use_speculative=_use_spec)
            # Merge hardware-profile options if available
            if self._inference_profile:
                options.update(self._inference_profile.recommended_ollama_options)
            if options:
                kwargs["options"] = options

        try:
            t0 = time.monotonic()
            warm_pool.mark_call_start(model)
            ...
            warm_pool.record_call(model, (time.monotonic() - t0) * 1000)
            return text
