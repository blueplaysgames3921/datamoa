...
class ConfidenceAgent(BaseAgent):
    ...
    async def _run(self, record: PipelineRecord) -> PipelineRecord:
        ...
        field_summary = {
            ...
            for fc in record.parsed.field_confidences
        }

        prompt = f"""Document type: {record.parsed.document_type}
Parse notes: {record.parsed.parse_notes}

Field extractions:
{json.dumps(field_summary, indent=2)}

Total fields: {len(record.parsed.fields)}
Flagged fields: {sum(1 for fc in record.parsed.field_confidences if fc.flagged)}
Average field confidence: {
    sum(fc.confidence for fc in record.parsed.field_confidences) / len(record.parsed.field_confidences)
    if record.parsed.field_confidences else 0.0
:.2f}
