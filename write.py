...
# Per-file locks so concurrent records writing to the same CSV destination
# (multiple records can be in-flight at once — see max_concurrent_records)
# don't race on the "does the file exist yet / do I need to write a header"
# check-and-write, which would otherwise produce duplicate header rows or
# interleaved writes.
_CSV_FILE_LOCKS: dict[str, asyncio.Lock] = defaultdict(asyncio.Lock)
...
class WriteAgent(BaseAgent):
    ...
    async def _run(self, record: PipelineRecord) -> PipelineRecord:
        ...
        raw_data = record.resolved_data if record.resolved_data is not None else (record.parsed.fields if record.parsed else {})
        ...
        if not write_data:
            record.write_result = WriteResult(
                ...
                error="No data to write after applying field mapping and exclusions. Check field_mapping and exclude_fields in destination config.",
            )
            return record
...
Use the appropriate write tool."""
    ...
    async def _write_csv(self, args: dict) -> WriteResult:
        ...
        # Use the resolved absolute path as the lock key so two records
        # writing to "the same file" (possibly via different relative
        # paths) serialize correctly.
        lock_key = str(path.resolve())
        lock = _CSV_FILE_LOCKS[lock_key]
