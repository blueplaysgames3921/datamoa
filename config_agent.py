...
def _build_system_prompt(
    ...
    return """You are the Configuration Agent for DataMoA, a professional multi-agent data entry system.
...
class ConfigAgent:
            ...
            for attempt in range(3):
                try:
                    ...
                    if invalid_ids:
                        ...
                        retry_msg = (
                            "Your response contained invalid model IDs that are not in the available list:\n"
                            f"{invalid_list}\n\n"
                            f"Valid model IDs (use ONLY these):\n{valid_list}\n\n"
                            "Correct ONLY the invalid assignments. Keep valid ones unchanged. Return the full JSON."
                        )
                        if attempt < 2:
    ...
    def _build_user_message(
        ...
        return """Assign DataMoA agent models for this exact user setup.
